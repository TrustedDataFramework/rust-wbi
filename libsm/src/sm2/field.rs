...
impl FieldElem {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        for i in 0..8 {
            ret.extend_from_slice(&self.value[i].to_be_bytes());
        }
        ret
    }
    pub fn from_bytes(x: &[u8]) -> FieldElem {
        ...
        for i in 0..8 {
            let mut y: [u8; 4] = [0, 0, 0, 0];
            &y.copy_from_slice(&x[i*4..(i* 4 + 4)]);
            elem.value[i] = u32::from_be_bytes(y);
        }
        elem
    }
...
mod tests {
    ...
    fn rand_elem() -> FieldElem {
        let mut rng = rand::rngs::SmallRng::seed_from_u64(128888);
