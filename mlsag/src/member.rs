...
impl Member {
    pub fn new_signer_from_bytes(seed: u64, private_keys: Vec<Vec<u8>>) -> Result<Self, &'static str> {
        let z: Result<Vec<Scalar>, &'static str> = private_keys.into_iter().map( |x| { 
            if x.len() != 32 {
    ...
    pub fn new_decoy(seed: u64, public_keys: Vec<RistrettoPoint>) -> Self {
        let num_public_keys = public_keys.len();
        let responses = generate_rand_scalars(seed, num_public_keys);
...
fn generate_rand_scalars(seed: u64, num: usize) -> Vec<Scalar> {
...
mod test {
    ...
    fn test_new() {
        ...
        let scalars = generate_rand_scalars(2, num_private_keys);

        let signer = Member::new_signer(4, scalars);
